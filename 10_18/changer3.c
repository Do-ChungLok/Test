...
int counting_lines(char *ptr)
{
	...
	if (ptr != p-1) 	    // 'ptr'값과 'p-1'값이 같지 않으면 line 갯수 +1 카운트 // ptr : 최종문자, p-1 : 이전문자
	total_lines++;
...
int main(int argc, char *argv[]) {
	...
	printf(" 입력할 라인 시작 지점 : %c\n",*start);
	printf(" 입력할 라인의 밑라인 시작 지점 : %c\n",*start_next);
	...
	char *a, *b;
	if (input_str_len < before_str_len) {     // '입력한(변경할) 문자열 길이' < '변경 이전 문자열 길이' 조건일때 
		memcpy(start, str, input_str_len);  // '입력한(변경할) 문자열 길이' 만큼 '입력한(변경할) 문자열'을 '라인 시작지점'에 복사
	      ...
	      // --> '입력 문자열' 바로 뒤로 '변경 이전 문자열'을 땡겨서(이어서) 붙임
	...
	else {					   // '입력한(변경할) 문자열 길이' >= '변경 이전 문자열 길이' 조건일때
		b = addr + statbuf.st_size;	   // b = addr(1234\n abcd\n qwer\0) + 매핑파일 크기(=15)
